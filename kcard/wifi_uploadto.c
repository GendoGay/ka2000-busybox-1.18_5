...
#include "libbb.h"
#include "common.h"
#else  /* for pc linux test CGI */
#define wifi_upload_main main
...
void do_exit(int flag)
{
    system("sync"); // umount and mount sd to refresh the FAT of ARM before checking the control image
    enable_kcard_call();
    exit(flag);
}
...
void ShowOkPage(void)
{
    ...
    while (fgets(Line,BUFSIZ,Fin) != NULL)
    {
        ...
        while (*p1 != '\0')
        {
            ...
            if (strnicmp(p1,"<insert Path>",13) == 0)
            {
                *p1 = '\0';
                sprintf(s1,"%s%s%s",Line,LastFullName,p1 + 13);
                strcpy(Line,s1);
            }
            p1++;
        }
...
void LoadConfig(char *ProgramPath, char *ConfigID)
{
    ...
    Fin = fopen("../page/upload.cfg","rt");
    if (Fin == NULL) 
		ShowBadPage("I could not open the configuration file.");
        ...
        *Name = *Value = '\0';
        ...
        if (stricmp(Name,"Config") == 0)
        {
            ...
            if (Debug > 0)
            {
                if (Accept == YES)
                {
                    fprintf(stdout,"<h2><font  face='Arial' color='#736F6E' size='4' style='font-weight: normal;margin-right:140px'>Loading configuration</font></h2>\"%s\"\n",Value);
				   ...
				   char *ToPath)
{
    ...
    *InputLength = *InputLength - strlen(Boundary);
	...
	if (ToPath != "")
		sprintf(Path,"%s/%s",ToPath,UpFileName);
	else
		sprintf(Path,"%s%s",Root,UpFileName);
	//sprintf(s1, "<P>Root=%s, UpFileName=%s, FileName=%s,Path=%s\n", Root, UpFileName, FileName, Path);
	//WriteLogLine(s1);
...
//------------------------------------------------------------------------------
int upload_raw_file(int argc, char **argv, char *to_path)
{
    char lenable[200] = {0, };
    ...
    system("refresh_sd"); // umount and mount sd to refresh the FAT of ARM before checking the control image
    ...
    *LastFileName = '\0';
    ...
    if (Debug > 0) fprintf(stdout,"Content_Length = %ld<br>\n",InCount);
    ...
    *p1 = '\0';
    ...
    disable_kcard_call();  //KCard: start to write picture into sd card.
    ...
    while (RealCount > 0) AcceptSegment(&Here,&RealCount,Boundary,argv[0],to_path);
    if (Debug > 0) fprintf(stdout,"</table>\n</center>\n");
    ...
    if (Debug > 0)
    {
        fprintf(stdout,"<p><center><h2><font  face='Arial' color='#736F6E' size='4' style='font-weight: normal;margin-right:140px'>Finished</font></h2></center>\n");
    }

    system("sync");        //KCard: force to write image into sd card
    enable_kcard_call();   //KCard: re-enable kcard application.
...
//------------------------------------------------------------------------------
static char *get_query_string(const char *token, char *dest)
{
    char* get_data = getenv("QUERY_STRING");

	if (get_data != NULL && get_data[0] != 0 && strlen(get_data) > 3)
	{
...
//------------------------------------------------------------------------------
int wifi_upload(int argc, char **argv)
{
	char dir[1024];

    memset(dir, 0, 1024);
	get_query_string("p=", dir);
	if (dir[0] == 0)
		strcpy(dir, "/mnt/sd/DCIM/101_WIFI/");
	return upload_raw_file(argc, argv, dir);
}
//------------------------------------------------------------------------------
int wifi_upload_main(int argc, char **argv) MAIN_EXTERNALLY_VISIBLE;
