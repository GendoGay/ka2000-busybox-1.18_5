...
void ShowOkPage(void)
{
    ...
    if (Debug == 0)
    {
        ...
        fprintf(stdout,"Content-type: text/html\n\n");
        fprintf(stdout,"<html>\n<head>\n");
        fprintf(stdout,"<meta http-equiv=\"content-language\" content=\"zh-tw\">\n");
...
#if 0
    memset(dir, 0, 1024);
    if (get_query_string("p=", dir) != 0)
    {                         
        strcpy(path, dir);
    }
#endif
    sprintf(cmd, "if [ ! -d %s ]; then mkdir -p %s && sync fi ", path);
    system(cmd);           
    
    return upload_raw_file(argc, argv, path);
}
