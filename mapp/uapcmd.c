...
{
    printf("\nUsage : sys_cfg_11n [ENABLE] [HTCAP] [AMPDU]\n");
...
get_sys_cfg_11n(HTCap_t * pHtCap)
{
    ...
    if (ret == UAP_SUCCESS) {
        ...
        if (cmd_buf->result == CMD_SUCCESS) {
            ...
            ret = UAP_FAILURE;
            printf("ERR:Could not get htcapibility!\n");
        }
    } else {
        printf("ERR:Command sending failed!\n");
        ret = UAP_FAILURE;
...
apcmd_sys_cfg_11n(int argc, char *argv[])
{
    ...
    if ((argc > 1) && (IS_HEX_OR_DIGIT(argv[1]) == UAP_FAILURE)) {
        printf("ERR: Only Number values are allowed\n");
        print_sys_cfg_11n_usage();
        return;
    }
    if ((argc > 2) && (IS_HEX_OR_DIGIT(argv[2]) == UAP_FAILURE)) {
        printf("ERR: Only Number values are allowed\n");
        print_sys_cfg_11n_usage();
        return;
    }
