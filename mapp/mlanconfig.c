 ...
 */
int
mlan_get_priv_ioctl(char *ioctl_name, int *ioctl_val, int *subioctl_val)
{
    int retval;
...
process_host_cmd(int argc, char *argv[])
{
    ...
    if (mlan_get_priv_ioctl("hostcmd",
    ...
    ret = mlan_process_host_cmd_resp(buf);
...
process_arpfilter(int argc, char *argv[])
{
    ...
    if (mlan_get_priv_ioctl("arpfilter",
                       &ioctl_val, &subioctl_val) == MLAN_STATUS_FAILURE) {
        return -EOPNOTSUPP;
    }
...
process_qconfig(int argc, char *argv[])
{
    ...
    if (mlan_get_priv_ioctl("qconfig",
                       &ioctl_val, &subioctl_val) == MLAN_STATUS_FAILURE) {
        return -EOPNOTSUPP;
    }
...
process_wmm_qstatus(int argc, char *argv[])
{
    ...
    if (mlan_get_priv_ioctl("qstatus",
                       &ioctl_val, &subioctl_val) == MLAN_STATUS_FAILURE) {
        return -EOPNOTSUPP;
    }
...
process_wmm_ts_status(int argc, char *argv[])
{
    ...
    if (mlan_get_priv_ioctl("ts_status",
                       &ioctl_val, &subioctl_val) == MLAN_STATUS_FAILURE) {
        return -EOPNOTSUPP;
    }
 ...
 *  
 *  @param s			a pointer to mlan_stack_t structure
 *  
 ...
 */
static int
push_n(mlan_stack_t * s, t_u8 nbytes, t_u8 * val)
{
 ...
 *  @param stack		A pointer to mlan_stack_t structure
 *  @return			MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int
str2bin(char *str, mlan_stack_t * stack)
{
    int first_time;
    char *opstr;
    ...
    memset(stack, 0, sizeof(mlan_stack_t));
    first_time = TRUE;
...
int mlanconfig_main(int argc, char **argv) MAIN_EXTERNALLY_VISIBLE;
int mlanconfig_main(int argc UNUSED_PARAM, char **argv UNUSED_PARAM)
{
    t_s32 cmd;
