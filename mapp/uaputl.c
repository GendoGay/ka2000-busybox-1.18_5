...
print_hscfg_usage(void)
{
    ...
    printf("\n         GPIO: the pin number of GPIO used to wakeup the host");
    printf
...
apcmd_sys_reset(int argc, char *argv[])
{
    ...
    send_power_mode_ioctl(&pm);
    send_bss_ctl_ioctl(UAP_BSS_RESET);
    return;
}
...
apcmd_bss_start(int argc, char *argv[])
{
    ...
    send_bss_ctl_ioctl(UAP_BSS_START);
...
apcmd_bss_stop(int argc, char *argv[])
{
    ...
    send_bss_ctl_ioctl(UAP_BSS_STOP);
    return;
}
...
apcmd_sys_config_profile(int argc, char *argv[])
{
    ...
    while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
        ...
        if (strcmp(args[0], "country") == 0) {
            ...
            /* Send the command */
            if (uap_ioctl((t_u8 *) cmd_buf, &cmd_len, cmd_len) == UAP_SUCCESS) {
                if (cmd_buf->result != CMD_SUCCESS) {
                    printf("ERR: Failed to set the configuration!\n");
                    goto done;
                }
        ...
        if (strcmp(args[0], "PwkCipherWPA") == 0) {
            if (arg_num == 1) {
                ...
                if (protocol == PROTOCOL_WPA2) {
                    printf
                        ("ERR:Illegal protocol.Must be either WPA or Mixed.\n");
                    goto done;
                }
        ...
        if (strcmp(args[0], "PwkCipherWPA2") == 0) {
            if (arg_num == 1) {
                ...
                if (protocol == PROTOCOL_WPA) {
                    printf
                        ("ERR:Illegal protocol.Must be either WPA2 or Mixed.\n");
                    goto done;
                }
    ...
    if (ret == UAP_SUCCESS) {
        ...
        if (cmd_buf->result == CMD_SUCCESS) {
            ...
            print_tlv(buf + sizeof(apcmdbuf_sys_configure),
                      ...
                      BUF_HEADER_SIZE);
            apcmd_addbapara(1, argv_dummy);
            apcmd_aggrpriotbl(1, argv_dummy);
            apcmd_addbareject(1, argv_dummy);
            printf("\n802.11D setting:\n");
...
check_sys_config(t_u8 * buf, t_u16 len)
{
    ...
    while (tlv_buf_left >= (int) sizeof(tlvbuf_header) && (ret != UAP_FAILURE)) {
        ...
        case MRVL_CHANNELCONFIG_TLV_ID:
            ...
            ret = check_channel_validity_11d(channel_tlv->chan_number);
            break;
        ...
        case MRVL_TX_POWER_TLV_ID:
            ...
            ret = check_tx_pwr_validity_11d(txpower_tlv->tx_power_dbm);
            break;
        }
        ...
        if (is_ap_config == 1) {
            if (strcmp(args[0], "}") == 0) {
                ...
                if (((pwkcipher_wpa >= 0) || (pwkcipher_wpa2 >= 0)) &&
                    (gwkcipher >= 0)) {
                    if (protocol == PROTOCOL_WPA2)
                        pairwisecipher = pwkcipher_wpa2;
                    else
                        pairwisecipher = pwkcipher_wpa;

                    if (is_cipher_valid(pairwisecipher, gwkcipher) !=
                        UAP_SUCCESS) {
                        printf
                            ("ERR:Wrong group and pairwise cipher combination!\n");
                        retval = UAP_FAILURE;
                        goto done;
                    }
 ...
 *  @return         0 or 1
 */
int uaputl_main(int argc, char **argv) MAIN_EXTERNALLY_VISIBLE;
int uaputl_main(int argc UNUSED_PARAM, char **argv UNUSED_PARAM)
{
    int opt, i;
