...
/* Pointed out 3-Oct-1999 by Paul Saab <paul@mu.org> */
	...
	fprintf(stderr, "%s : socket %d, "
		    "IP %s:%u\n",
		    __FUNCTION__, fd, conn->remote_ip_addr ,
		    conn->remote_port);
#endif
...
void process_requests(int server_sock)
{
    ...
    static request req_pre = { .status = -1 };
    int changed_flag = 0;

    /* unconditional get_request for support of multiple request */
    //if (pending_requests) {
...
#ifdef DEBUG_PROGRESS
//fprintf(stderr, "%d", current->fd);
	if (req_pre.status != -1) {
		if (strcmp(current->remote_ip_addr,req_pre.remote_ip_addr) ||
				current->remote_port != req_pre.remote_port ||
				current->status != req_pre.status) {
			DEBUG(DEBUG_REQUEST) {

				log_error_doc(current);

				if (current->status != req_pre.status) 
					fprintf(stderr, "process_requests - "
							"Status: %d CGI-type %d, preStatus %d\n", current->status,current->cgi_type,req_pre.status);

				if (strcmp(current->remote_ip_addr,req_pre.remote_ip_addr))
					fprintf(stderr, "process_requests - "
							"Status: %d ,preIP %s\n", current->status,req_pre.remote_ip_addr);

				if (current->remote_port != req_pre.remote_port) 
					fprintf(stderr, "process_requests - "
							"Status: %d ,Port %u prePort %u\n", current->status,current->remote_port,req_pre.remote_port);
			}
...
#ifdef DEBUG_PROGRESS
		if (changed_flag == 1 && trailer != current) {
			DEBUG(DEBUG_REQUEST) {
				log_error_doc(current);
				fprintf(stderr, "process_requests - block"
						"Status: %d CGI-type %d\n", current->status,current->cgi_type);
			}
		}
#endif
...
#ifdef DEBUG_PROGRESS
		log_error_doc(current);
		    fprintf(stderr, "process_requests - complete"
			    "Status: %d CGI-type %d\n", current->status,current->cgi_type);
		req_pre = *current;
#endif
...
//			fprintf(stderr, "header \"%s\" AUTHORIZATION contain ':'%s\n", line,value);
