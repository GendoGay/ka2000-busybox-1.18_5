...
void auth_add(char *directory,char *file)
{
	...
	if (new_a->authfile == NULL) {
		fprintf(stderr,"Auth file %s open failed for %s !\n",
				file, directory);
	}
		...
		if (pwd)
		{
			*pwd++=0;
			if (!strcmp(temps,user))
			{
				...
				DBG(printf("Passowrd [%s] not matched with %s\n",pwd,encoded_passwd);)
			} else {
				DBG(printf("User %s not matched with %s\n",user,temps);)
				return 2;
			}
		}
	}
...
int auth_authorize(request * req)
{
  ...
  DBG(printf("auth_authorize reuqest_uri=[%s]\n",req->request_uri);)
  if (req->authorization) {
	  DBG(printf("auth_authorize req_authorization =[%s]\n",req->authorization);)
  }
  ...
  while (current) {
		DBG(printf("auth_authorize: current dir =[%s]\n",current->directory);)
		if (!memcmp(req->request_uri, current->directory,
			...
			if (current->directory[current->dir_len - 1] != '/' &&
				...
				DBG(printf("auth_authorize: current dir =[%s] break\n",current->directory);)
				break;
			}
			if (req->authorization)
			{
				...
				{
					fprintf(stderr, "AUTH: authfile pointer is invalid\n");
					send_r_error(req);
					return 0;
				}
				...
				if ( (pwd = strchr(auth_userpass,':')) == 0 )
				{
					...
					DBG(printf("auth_authorize: No user:pass in Basic auth\n");)
					send_r_bad_request(req);
					return 0;
				}
				
				DBG(printf("auth_authorize: user:pass [%s] pass [%s]\n",auth_userpass,pwd);)
				*pwd++=0;

				denied = auth_check_userpass(auth_userpass,pwd,current->authfile);
				DBG(printf("auth_authorize: denied %d\n",denied);)
#ifdef SECURITY_COUNTS
				...
				DBG(printf("Curent Client %s ,req->remote_ip_addr %s\n", current_client, req->remote_ip_addr);)
				if (strcmp(current_client, req->remote_ip_addr) != 0) {
					char tmp_buf[256] = {0};
					if (auth_exec_file != NULL) {
						sprintf(tmp_buf, "%s %s %s",auth_exec_file, req->request_uri, req->remote_ip_addr);
						DBG(printf("AuthExec %s\n",tmp_buf);)
						system(tmp_buf);
					}
					DBG(printf("Authentication successful for %s from %s\n", auth_userpass, req->remote_ip_addr);)
			...
			}else
			{
				...
				DBG(printf("No credentials were supplied\n");)
				send_r_unauthorized(req,server_name);
				return 0;
			}
		}
