...
char *uad = (char)NULL;
char *config_file_ssmtp = (char)NULL;		/* alternate configuration file */
...
/*
read_config_ssmtp() -- Open and parse config file and extract values of variables
*/
bool_t read_config_ssmtp()
{
	char buf[(BUF_SZ + 1)], *p, *q, *r;
	FILE *fp;

	if(config_file_ssmtp == (char *)NULL) {
		config_file_ssmtp = strdup(CONFIGURATION_FILE);
		if(config_file_ssmtp == (char *)NULL) {
			die("parse_config() -- strdup() failed");
		}
	}

	if((fp = fopen(config_file_ssmtp, "r")) == NULL) {
		return(False);
	}
...
int ssmtp(char *argv[])
{
	...
	if(read_config_ssmtp() == False) {
		log_event(LOG_INFO, "%s not found", config_file_ssmtp);
	}
...
*/
			...
			case 'C':
				if((!argv[i][(j + 1)]) && argv[(i + 1)]) {
					config_file_ssmtp = strdup(argv[(i + 1)]);
					if(config_file_ssmtp == (char *)NULL) {
						die("parse_options() -- strdup() failed");
					}
					add++;
					...
					config_file_ssmtp = strdup(argv[i]+j+1);
					if(config_file_ssmtp == (char *)NULL) {
						die("parse_options() -- strdup() failed");
					}
				}
...
*/
int ssmtp_main(int argc, char **argv) MAIN_EXTERNALLY_VISIBLE;
int ssmtp_main(int argc, char **argv)
{
	char **new_argv;
